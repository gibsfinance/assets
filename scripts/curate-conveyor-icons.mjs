#!/usr/bin/env node
/**
 * Curate conveyor belt icons by fetching each from staging
 * and filtering out low-res rasters (< 64x64).
 * SVGs always pass. Outputs a filtered list of icon paths.
 */

const BASE_URL = process.env.BASE_URL || 'https://staging.gib.show'
const MIN_DIM = 64 // minimum width and height for rasters
...
// GIF dimensions (bytes 6-9, little-endian)
...
const ICON_PATHS = [
  ...
  '/image/56/0xc636782a837feee37ccc29d58bdba861ca1d83df7c',
...
async function checkIcon(path) {
  ...
  try {
    ...
    if (!res.ok) return { path, status: 'error', reason: `HTTP ${res.status}` }

    const contentType = res.headers.get('content-type') || ''

    // SVGs always pass
    if (contentType.includes('svg')) {
      return { path, status: 'pass', format: 'svg', width: null, height: null }
    }

    const buf = Buffer.from(await res.arrayBuffer())
    const dims = getDimensions(buf, contentType)

    if (!dims) {
      return { path, status: 'unknown', reason: `could not parse dimensions (${contentType}, ${buf.length} bytes)` }
    }

    if (dims.width < MIN_DIM || dims.height < MIN_DIM) {
      return { path, status: 'reject', reason: `${dims.width}x${dims.height} < ${MIN_DIM}x${MIN_DIM}`, width: dims.width, height: dims.height }
    }

    return { path, status: 'pass', format: contentType.split('/')[1], width: dims.width, height: dims.height }
  } catch (err) {
    return { path, status: 'error', reason: err.message }
  }
}

async function main() {
  console.error(`Checking ${ICON_PATHS.length} icons against ${BASE_URL} (min ${MIN_DIM}x${MIN_DIM})...\n`)

  const results = []
  for (let i = 0; i < ICON_PATHS.length; i += CONCURRENCY) {
    const batch = ICON_PATHS.slice(i, i + CONCURRENCY)
    const batchResults = await Promise.all(batch.map(checkIcon))
    results.push(...batchResults)
    const done = Math.min(i + CONCURRENCY, ICON_PATHS.length)
    process.stderr.write(`\r  ${done}/${ICON_PATHS.length}`)
  }
  console.error('\n')
  ...
  console.error(`  PASS: ${passed.length}`)
  console.error(`  REJECT (too small): ${rejected.length}`)
  console.error(`  ERROR (404/network): ${errors.length}`)
  console.error(`  UNKNOWN (unparseable): ${unknown.length}`)
  ...
  if (unknown.length > 0) {
    console.error(`\nUnknown format icons:`)
    for (const r of unknown) {
      console.error(`  ${r.path} — ${r.reason}`)
    }
  }

  // Output the curated list as JS array (only passed icons)
  const curated = passed.map(r => r.path).sort()
