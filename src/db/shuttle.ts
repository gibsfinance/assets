...
export const generateCompleteHomeToForeign = async <T extends MinimalEventData>(
  ...
  const messageHashes = _(events).map('messageHash').uniq().value()
  ...
  const originatingUserRequests = await t(eventNames.userRequestForSignature).whereIn('hash', messageHashes)
  const signatures = await t(eventNames.signedForUserRequest).whereIn('messageHash', messageHashes)
  ...
  await t.transaction(async (tx) => {
    ...
    await insertIntoTable(tx, tableNames.relayRequest, requestedRelays, ['relayRequestId'], ['relayRequestId'])
  })
}
