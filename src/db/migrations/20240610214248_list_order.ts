...
const compositeId = utils.compositeId(tableNames.listOrder, 'listOrderId', ['providerId', 'key'])

export async function up(knex: Knex): Promise<void> {
  const exists = await knex.schema.withSchema(userConfig.database.schema).hasTable(tableNames.listOrder)
  if (!exists) {
    ...
    //   })
    await knex.schema.withSchema(userConfig.database.schema).createTable(tableNames.listOrder, (t) => {
    ...
    await knex.schema.withSchema(userConfig.database.schema).createTable(tableNames.listOrderItem, (t) => {
      t.text('listOrderId')
        .notNullable()
        .index()
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema.withSchema(userConfig.database.schema).dropTableIfExists(tableNames.listOrderItem)
  await compositeId.down(knex)
  await utils.autoUpdateTimestamp.down(knex, tableNames.listOrder)
  await knex.schema.withSchema(userConfig.database.schema).dropTableIfExists(tableNames.listOrder)
}
