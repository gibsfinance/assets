...
export async function up(knex: Knex): Promise<void> {
  const exists = await knex.schema.withSchema(userConfig.database.schema).hasTable(tableNames.tag)
  if (!exists) {
    ...
    await knex.schema.withSchema(userConfig.database.schema).createTable(tableNames.tag, (t) => {
      t.text('providerId')
        .index()
        .notNullable()
    ...
    await utils.autoUpdateTimestamp.up(knex, tableNames.tag)
  }
}

export async function down(knex: Knex): Promise<void> {
  await utils.autoUpdateTimestamp.down(knex, tableNames.tag)
  await knex.schema.withSchema(userConfig.database.schema).dropTableIfExists(tableNames.tag)
}
