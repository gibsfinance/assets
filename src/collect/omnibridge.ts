...
export const collectByBridgeConfig = async (config: BridgeConfig) => {
  await Promise.all([config.home, config.foreign].map(async (fromConfig) => {
    ...
    await iterateOverRange(fromBlock, latestBlock.number, async (fromBlock, toBlock) => {
      ...
      const collectedData = await Promise.all(events.map(async (event) => {
        ...
        ] = await erc20Read(toConfig.chain, toClient, bridged)
        const metadata = {
      ...
      const collectedDataForTokens = new Map<string, {
        decimals: number;
        symbol: string;
        name: string;
      ...
      await db.transaction(async (tx) => {
        for (const event of events) {
          const [native, bridged] = await Promise.all([
            [fromConfig.chain.id, event.args.native] as const,
            [toConfig.chain.id, event.args.bridged] as const,
