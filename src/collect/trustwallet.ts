...
export const collect = async (): Promise<string[]> => {
  ...
  return await utils.limit.map(blockchainFolders, async (folder) => {
    try {
      ...
      const { logoURI, chainId, entries } = await entriesFromAssets(folder, utils.removedUndesirable(assets))
      const result = await utils.providerLink.update('trustwallet', chainId, _.compact(entries), {
        //
      })
      return result.path
    } catch (err) {
...
const entriesFromAssets = async (blockchainKey: string, assets: string[]) => {
  ...
  await utils.networkImage.update(chainId, networkLogo)
  return {
