...
fetch(\`\${baseUrl}/image/1\`)
  ...
  const each_array = ensure_array_like(endpoints.tokenInfo);
  const each_array_1 = ensure_array_like(endpoints.imageEndpoints);
  $$payload.out += `<div class="container mx-auto max-w-4xl space-y-12 p-8"><div class="space-y-4 text-center"><h1 class="h1">API Documentation</h1> <p class="text-lg">Complete reference for the Gib Assets API</p></div> <section class="space-y-6"><h2 class="h2">Token Information Endpoints</h2> <div class="card variant-ghost"><div class="space-y-4 p-4"><!--[-->`;
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    let endpoint = each_array[$$index];
    $$payload.out += `<div class="card variant-soft p-4"><div class="flex flex-col gap-2"><code class="font-mono text-lg text-primary-500">${escape_html(endpoint.path)}</code> <p class="text-sm">${escape_html(endpoint.description)}</p></div></div>`;
  }
  $$payload.out += `<!--]--></div></div></section> <section class="space-y-6"><h2 class="h2">Image Endpoints</h2> <div class="card variant-ghost"><div class="space-y-4 p-4"><!--[-->`;
  for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
    let endpoint = each_array_1[$$index_1];
    $$payload.out += `<div class="card variant-soft p-4"><div class="flex flex-col gap-2"><code class="font-mono text-lg text-primary-500">${escape_html(endpoint.path)}</code> <p class="text-sm">${escape_html(endpoint.description)}</p></div></div>`;
  }
  $$payload.out += `<!--]--></div></div></section> <section class="space-y-6"><h2 class="h2">Features</h2> <div class="grid gap-4 md:grid-cols-2"><div class="card variant-soft p-4"><h3 class="h3 mb-2">Token Management</h3> <ul class="list-inside list-disc space-y-2"><li>Token list management</li> <li>Token data aggregation</li> <li>Token statistics tracking</li></ul></div> <div class="card variant-soft p-4"><h3 class="h3 mb-2">Image Handling</h3> <ul class="list-inside list-disc space-y-2"><li>Token image serving and caching</li> <li>Network/chain images</li> <li>Fallback mechanisms</li></ul></div> <div class="card variant-soft p-4"><h3 class="h3 mb-2">Network Support</h3> <ul class="list-inside list-disc space-y-2"><li>Multiple blockchain networks</li> <li>Chain-specific data</li> <li>Cross-chain compatibility</li></ul></div> <div class="card variant-soft p-4"><h3 class="h3 mb-2">Data Management</h3> <ul class="list-inside list-disc space-y-2"><li>Database operations</li> <li>Configuration management</li> <li>Efficient caching</li></ul></div></div></section> <section class="space-y-6"><h2 class="h2">Example Usage</h2> <div class="card variant-ghost p-6"><pre class="overflow-x-auto text-sm"><code>${escape_html(codeExamples)}</code></pre></div></section> <div class="text-center"><a href="#/wizard" class="btn bg-[#00DC82] text-black hover:bg-[#00DC82]/80"><i class="fas fa-hat-wizard mr-2"></i> Wizard</a></div></div>`;
}
export {
  _page as default
};
