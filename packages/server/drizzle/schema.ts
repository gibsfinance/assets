import { pgTable, serial, varchar, integer, timestamp, index, unique, text, foreignKey, jsonb, smallint, numeric, boolean, bigint, uuid, primaryKey } from "drizzle-orm/pg-core"
...
export const listTag = pgTable("list_tag", {
...
export const metadata = pgTable("metadata", {
	...
	providedId: unknown("provided_id"),
...
export const token = pgTable("token", {
	...
	// TODO: failed to parse database type 'citext'
	providedId: unknown("provided_id").notNull(),
	name: text().notNull(),
	symbol: text().notNull(),
	decimals: smallint().default(sql`'0'`).notNull(),
...
export const listOrder = pgTable("list_order", {
	providerId: text("provider_id").notNull(),
	key: text().notNull(),
	type: text().notNull(),
	name: text(),
...
export const list = pgTable("list", {
	...
	name: text(),
	description: text(),
	patch: smallint().default(sql`'0'`).notNull(),
	minor: smallint().default(sql`'0'`).notNull(),
	major: smallint().default(sql`'0'`).notNull(),
	imageHash: text("image_hash"),
...
export const bridgeLink = pgTable("bridge_link", {
...
export const listToken = pgTable("list_token", {
...
export const bridge = pgTable("bridge", {
	...
	// TODO: failed to parse database type 'citext'
	homeAddress: unknown("home_address").notNull(),
	foreignNetworkId: text("foreign_network_id").notNull(),
	// TODO: failed to parse database type 'citext'
	foreignAddress: unknown("foreign_address").notNull(),
	bridgeId: text("bridge_id").primaryKey().notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	currentForeignBlockNumber: bigint("current_foreign_block_number", { mode: "number" }).default(sql`'0'`).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	currentHomeBlockNumber: bigint("current_home_block_number", { mode: "number" }).default(sql`'0'`).notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	...
	index("bridge_currentforeignblocknumber_index").using("btree", table.currentForeignBlockNumber.asc().nullsLast().op("int8_ops")),
	index("bridge_currenthomeblocknumber_index").using("btree", table.currentHomeBlockNumber.asc().nullsLast().op("int8_ops")),
...
export const cacheRequest = pgTable("cache_request", {
	key: text().primaryKey().notNull(),
	value: text().notNull(),
	expiresAt: timestamp("expires_at", { precision: 3, withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
	index().using("btree", table.key.asc().nullsLast().op("text_ops")),
]);

export const listSubmission = pgTable("list_submission", {
...
export const listOrderItem = pgTable("list_order_item", {
...
export const imageVariant = pgTable("image_variant", {
	...
	createdAt: timestamp("created_at", { precision: 3, withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
	lastAccessedAt: timestamp("last_accessed_at", { precision: 3, withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
	index("idx_image_variant_prune").using("btree", table.accessCount.asc().nullsLast().op("int4_ops"), table.lastAccessedAt.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
			columns: [table.imageHash],
			foreignColumns: [image.imageHash],
