      ...
      {references.map((ref, idx) => {
        ...
        return (
          <div key={ref.sourceList} className="relative flex items-center gap-1.5 py-1">
            ...
            {isLast && <div className="absolute left-0 top-0 h-1/2 w-px bg-gray-200 dark:bg-surface-3" />}
            <div className="w-3 flex-shrink-0" />
            <Image src={ref.imageUri} size={14} skeleton lazy shape="circle" className="rounded-full" />
            <span className="flex-1 truncate text-[10px] text-gray-500 dark:text-white/40">{ref.sourceList}</span>
            {ref.imageFormat && (
              <span
