...
export default function Studio() {
  ...
  const { isOpen: editorOpen, openNewEditor, openEditor, closeEditor } = useListEditor()
  ...
  const urlSelectChain = useCallback(
    (chainId: string | null) => {
      setSearchParams(
        (prev) => {
  ...
  const urlSelectToken = useCallback(
    (token: Token) => {
  ...
  return (
    <div className="h-screen">
      ...
      <div className="hidden lg:block h-full overflow-hidden">
        <div
          ...
          style={{
            ...
            transform: editorOpen ? 'translateX(0)' : 'translateX(calc(-100vw + 380px))',
          }}>
          {/* Left: List Editor */}
          ...
          <div className="h-full w-[380px] flex-shrink-0 border-x border-border-light dark:border-border-dark bg-white dark:bg-surface-base">
            <div className="h-full flex flex-col">
              <div className="flex items-center justify-between px-4 py-3 border-b border-border-light dark:border-border-dark">
                <Link
                  to="/"
                  className="font-heading text-xl font-bold text-gradient-brand hover:opacity-80 transition-opacity">
                  Gib.Show
                </Link>
              ...
              <div className="flex-1 overflow-y-auto">
                <StudioBrowser
                  onInspectToken={setInspectToken}
                  selectChain={urlSelectChain}
                  selectToken={urlSelectToken}
                />
              </div>
            </div>
          </div>
          ...
          handle={
            selectedToken ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400 dark:text-gray-500">{selectedToken.symbol}</span>
              </div>
            ) : undefined
          }>
          <StudioConfigurator />
      ...
      {/* Token Detail Modal */}
      <TokenDetailModal token={inspectToken} onClose={() => setInspectToken(null)} />
    </div>
  )
}
